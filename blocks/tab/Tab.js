export class Tab {
  ...
  showPanel(nextPanel) {
    if (nextPanel) {
      ...
      clearTimeout(this.changePanelTimer)
      this.changePanelTimer = setTimeout(() => {
  ...
  destroy() {
    ...
    this.panels.forEach(panel => panel.classList.remove(this.classes.panel, this.classes.panelIn, this.classes.panelActive))

    this.panelContainer.style.height = ''
